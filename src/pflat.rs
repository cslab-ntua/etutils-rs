...
fn main() {
    ...
    let thp = env::args().nth(4).unwrap().parse::<bool>().unwrap();
    let shuffle = env::args().nth(5).unwrap().parse::<bool>().unwrap();
    let nr_faults = env::args().nth(6).unwrap().parse::<usize>().unwrap();
    let flush = env::args().nth(7).unwrap().parse::<bool>().unwrap();

    let fault_size = if thp { *HPMD_SIZE } else { *PAGE_SIZE };
    ...
    println!(
        "allocating {}, {} VMAs, {} per VMA, threads: {}, thp: {}, shuffle: {}, faults: {}, faults per loop: {}, loops: {}",
        size_to_str(vma_size * nr_vmas),
        nr_vmas,
        size_to_str(vma_size),
        nr_threads, thp, shuffle, nr_faults, faults_per_loop, loops
    );
